...
export const Search = (props) => {
  ...
  const almostAllGenres = useMemo(() => allGenres.filter(g => !props.chosenGenres.includes(g)), [allGenres, props.chosenGenres]);
  const [searchTerm, setSearchTerm] = useState("");
  const htmlifyOption = (option) => (
    <div>
      <button onClick={() => props.setChosenGenres([...props.chosenGenres, option])}>{option}</button>
    </div>
  );
   
  const options = useMemo(() => {
    console.log(searchTerm);
    if (!searchTerm || searchTerm.length === 0 || props.chosenGenres.length > 2) {
      return [];
    }
