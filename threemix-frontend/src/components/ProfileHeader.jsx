...
import { profilePageId } from "../pages";

export const ProfileHeader = ({ setCurrentPageId }) => {
    ...
    <div className="dropdown" onMouseDown={(e) => e.preventDefault()}>
      <button
        onClick={() => {
          ...
          setCurrentPageId(profilePageId);
        }}
      >
        Profile
      </button>
  ...
  const profileFragment = auth.accessToken && profileInfo && (
    <>
  ...
  return <>{profileFragment || (!auth.accessToken && loginLink)}</>;
};
